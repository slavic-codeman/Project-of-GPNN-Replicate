...
def evaluation(output, target, test=False):
...
def criterion(output, target):
    weight_mask = torch.ones(target.size())
...
def main(args):
    ...
    model = units.LinkFunction('GraphConv', {'edge_feature_size': edge_feature_size, 'link_hidden_size': 512, 'link_hidden_layers': 2, 'link_relu': False})
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        epoch_error = validate(test_loader, model, criterion, evaluation, logger, args=args)
...
def validate(val_loader, model, criterion, evaluation, logger=None, args=None, test=False):
    ...
    for i, (edge_features, node_features, adj_mat, node_labels, sequence_ids, det_classes, det_boxes, human_num, obj_num) in enumerate(val_loader):
        ...
        error_ratio.update(evaluation(output, target, test=test).data[0], edge_features.size(0))
...
def parse_arguments():
    ...
    parser.add_argument('--log-root', default=os.path.join(paths.log_root, 'hico/graph'), help='log files path')
    parser.add_argument('--resume', default=os.path.join(paths.tmp_root, 'checkpoints/hico/graph'), help='path to latest checkpoint')
