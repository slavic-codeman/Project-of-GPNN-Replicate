...
def main(args):
    ...
    # Load data
    training_set, valid_set, testing_set, train_loader, valid_loader, test_loader = utils.get_cad_data(args, prediction=True if args.task == "prediction" else False)

    # Get data size and define model
    edge_features, node_features, adj_mat, node_labels, _ = training_set[0]
    ...
    model = models.GPNN_CAD(model_args, device=device).to(device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, logger, device, args=args)
        # test on validation set
        epoch_error = validate(valid_loader, model, criterion, logger, device, args=args)
    ...
    validate(test_loader, model, criterion, logger, device, args=args, test=True)
    print('Time elapsed: {:.2f}s'.format(time.time() - start_time))


def train(train_loader, model, criterion, optimizer, epoch, logger, device, args=None):
...
def validate(val_loader, model, criterion, logger=None, device=None, args=None, test=False):
    if args.visualize:
        result_folder = os.path.join(args.tmp_root, f'results/CAD/figures/{args.task}/')
    ...
    with torch.no_grad():
        for i, (edge_features, node_features, adj_mat, node_labels, sequence_ids, node_nums) in enumerate(val_loader):
            ...
            # Logs
            losses.update(criterion(pred_node_labels, node_labels).item(), edge_features.size(0))
    ...
    print(f'Subactivity {args.task} micro evaluation:', subact_micro_result)
    print(f'Subactivity {args.task} macro evaluation:', subact_macro_result)
    print(f'Affordance {args.task} micro evaluation:', aff_micro_result)
    print(f'Affordance {args.task} macro evaluation:', aff_macro_result)
    ...
    if logger is not None:
        ...
        logger.log_value(f'test_epoch_subactivity_f1_{args.task}', subact_macro_result[2])
        logger.log_value(f'test_epoch_affordance_f1_{args.task}', aff_macro_result[2])
...
def parse_arguments(paths):
    ...
    parser.add_argument('--task', default="prediction", choices=["prediction", "parsing"], help='prediction or parsing')
    ...
    parser.add_argument('--lr', type=lambda x: restricted_float(x, [1e-5, 1e-2]), default=5e-5, metavar='LR',
                        help='Initial learning rate [1e-5, 1e-2] (default: prediction 5e-5, parsing 1e-4)')
...
if __name__ == '__main__':
    ...
    args.log_root = os.path.join(paths.log_root, f'cad120/{args.task}')
    args.resume = os.path.join(paths.tmp_root, f'checkpoints/cad120/{args.task}')
