...
def validate(val_loader, model, criterion, logger=None, args=None, test=False):
    if args.visualize:
        result_folder = os.path.join(args.tmp_root, 'results/CAD/figures/prediction/')
    ...
    if logger is not None:
        ...
        logger.log_value('test_epoch_subactivity_f1_prediction', subact_macro_result[2])
        logger.log_value('test_epoch_affordance_f1_prediction', aff_macro_result[2])
