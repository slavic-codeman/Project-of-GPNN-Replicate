...
def append_results(pred_adj_mat, adj_mat, pred_node_labels, node_labels, pred_node_roles, node_roles, img_ids, boxes, human_nums, obj_nums, classes, all_results):
    ...
    np_pred_node_roles = pred_node_roles_prob.detach().cpu().numpy()
    ...
    # # TODO: debug metrics
    # np_node_labels = node_labels.detach().cpu().numpy()
    # np_node_roles = node_roles.detach().cpu().numpy()
    # np_adj_mat = adj_mat.detach().cpu().numpy()
...
def get_vcocoeval(args, imageset):
    return vsrl_eval.VCOCOeval(os.path.join(args.data_root,  'v-coco-master/data/vcoco/vcoco_{}.json'.format(imageset)),
                               os.path.join(args.data_root,  'v-coco-master/data/instances_vcoco_all_2014.json'),
                               os.path.join(args.data_root,  'v-coco-master/data/splits/vcoco_{}.ids'.format(imageset)))
...
def parse_arguments(paths):
    ...
    parser.add_argument('--log-root', default=os.path.join(paths.log_root, 'vcoco/parsing'), help='log files path')
