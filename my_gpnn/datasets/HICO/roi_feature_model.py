...
class HICO(torch.utils.data.Dataset):
    def __init__(self, root, input_imsize, transform, imageset):
        ...
        anno_file = os.path.join(root, f"{imageset}_annotations.mat")
        ...
        with open(os.path.join(config.Paths().project_root, f"{imageset}_all.txt")) as f:
    ...
    def __getitem__(self, index):
        ...
        roi = roi.astype(int)  # Updated for Python 3 compatibility
        image_dir = 'test2015' if 'test' in image_i else 'train2015'
        image_path = os.path.join(self.hico_path, 'images', image_dir, f"{image_i}.jpg")
        assert os.path.exists(image_path)

        original_img = cv2.imread(image_path)[:, :, ::-1]  # Replace scipy.misc with cv2
...
def main():
    ...
    dataset = HICO('path_to_hico_root', input_imsize, transform, 'train')
