...
def parse_features(paths, imageset):
    ...
    for i_image, image_id in enumerate(image_ids):
        ...
        if image_id not in unique_image_ids:
            try:
                bbox_features = np.load(os.path.join(feature_path, '{}_features.npy'.format(filename)),allow_pickle=True)
                det_classes = np.load(os.path.join(det_feature_path, '{}_classes.npy'.format(filename)),allow_pickle=True)
                det_boxes = np.load(os.path.join(det_feature_path, '{}_boxes.npy'.format(filename)),allow_pickle=True)
                det_features = np.load(os.path.join(det_feature_path, '{}_features.npy'.format(filename)),allow_pickle=True)
            except IOError:
            ...
            edge_features = np.load(os.path.join(save_data_path, '{}_edge_features.npy').format(filename),allow_pickle=True)
            node_features = np.load(os.path.join(save_data_path, '{}_node_features.npy').format(filename),allow_pickle=True)
