...
def main(args):
    ...
    if args.feature_type == 'vgg':
        ...
        model = feature_model.Resnet152(num_classes=len(metadata.action_classes))
    elif args.feature_type == 'densenet':
        model = feature_model.Densenet(num_classes=len(metadata.action_classes))
    ...
    criterion = torch.nn.CrossEntropyLoss(weight=torch.FloatTensor(metadata.action_class_weight).cuda())  # 确保权重在 GPU 上
    ...
    train_dataset = feature_model.VCOCO(args.data, input_imsize, transform, 'train')
    val_dataset = feature_model.VCOCO(args.data, input_imsize, transform, 'val')
    test_dataset = feature_model.VCOCO(args.data, input_imsize, transform, 'test')
